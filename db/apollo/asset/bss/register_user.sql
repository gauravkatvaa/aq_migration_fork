...
BEGIN
    ...
    DECLARE entittlements CURSOR FOR
        ...
        FROM tdevice_plan_quota where DEVICE_PLAN_ID = devicePlanIdParam and ACCOUNT_ID=accountIdParam;
    ...
    update migration_asset asset set asset.status=b'0' where asset.imsi=imsi;
...
-- Inventory upload end
    ...
    SET asset.status = b'1'
    WHERE asset.imsi = imsi;
	END IF;

IF LOWER(userStatus) != 'warm' AND LOWER(userStatus) != 'testready' THEN

	SELECT COUNT(*) INTO accountCount FROM tenterprise_account WHERE account_id = accountIdParam;
    SELECT COUNT(*) INTO planCount FROM tdevice_plan WHERE planId = devicePlanIdParam and accountId=accountIdParam;
    SELECT COUNT(*) INTO serviceCount from tservice_plan where SERVICE_PLAN_ID =servicePlanIdParam and account_id=accountIdParam;
...
-- User Registration start
        ...
        INSERT INTO tuser (
            ...
            '', @lastname, 'd24b29eeedde2f7fb73724270f0d2396',
            @email, 0, '', '',
            '', '', '', '',0,
            NULL, '',3, 0,
            NULL,NULL, NULL,
		...
		-- Retrieve the last inserted ID
		   SET @user_id = (SELECT LAST_INSERT_ID() AS last_inserted_id);
        ...
        INSERT INTO tuser_sim_card (
            ...
            STATUS_UPDATE_TIME, NEXT_RENEWAL_DATE
        ) VALUES (
            @user_id, imsi, msisdn, NULL, iccid, NULL,
            3, 'GBRHU', NULL, 'GBRHU', 0, 0, NULL, NULL, NULL, NULL, NULL, NULL, clientTypeValue,
            4, NULL, NULL, NULL, 0, 0, NULL, NULL, NULL, NULL, NULL, NULL, NULL, accountId,devicePlanId, servicePlanId, false, now(), @next_renewal
        );
...
-- User purchase start
    ...
    read_loop: LOOP
      ...
      --  SELECT cur_id, cur_account_id, cur_device_plan_id, cur_client, cur_volume, cur_volume_unit,
       --     cur_product_type, cur_entitlement_name, cur_service_type, cur_zone_group_id, cur_zone_id;
            ...
            SET @user_purchase_id=(SELECT MAX(id)+1 FROM tuser_purchase);
         ...
         SET @transaction_id = (SELECT LAST_INSERT_ID() AS last_inserted_id);
			...
			-- SET @data_bucket_id=(SELECT MAX(id) FROM tdata_bucket);
             SET @data_bucket_id = (SELECT LAST_INSERT_ID() AS last_inserted_id);
           INSERT INTO tuser_purchase (
                USER_ID, TYPE_ENTRY_ID, USER_PURCHASE_TYPE, EXPIRATION_DATE, RECURRING,
	...
	SET @user_purchase_id = (SELECT LAST_INSERT_ID() AS last_inserted_id);
     update ttransactionlog set USER_PURCHASE_ID=@user_purchase_id where id=@transaction_id;
     update tdata_bucket set USER_PURCHASE_ID=@user_purchase_id where id=@data_bucket_id;
    ...
    CLOSE entittlements;
			...
			if @plan_type ='RollingPoolPlan' then
				...
				if  @smsValue !=null AND @smsValue !=0 and @sms_id!=null and @sms_id !=0 then
					...
					SET @rollingConceptionOrder=@rollingConceptionOrder+1;
					INSERT INTO `tuser_purchase` (`USER_ID`, `TYPE_ENTRY_ID`, `USER_PURCHASE_TYPE`, `EXPIRATION_DATE`, `RECURRING`, `PAYMENT_RETRY_COUNT`, `PAYMENT_REFUSE_DATE`, `TYPE`, `VOLUME`, `DATA_BUCKET_ID`, `PURCHASE_BUNDLE_ID`, `PURCHASE_BUNDLE_TYPE`, `MASTER_BUNDLE_ID`, `ACTIVE`, `TRANSACTION_ID`, `ENTERPRISE_ID`, `CREATED`, `ACTIVATION_DATE`, `PRIMARY_USER_PURCHASE_ID`, `RECURRING_COUNTER`, `SELECTED_NETWORK_GROUP_ID`, `THROTTLING_DATA_BUCKET_ID`, `RESET_VALIDITY_TERM_ENABLED`, `RESET_DATE`, `CAPPED`, `EXPIRED_NOTIF`, `ACTIVATION_NOTIF`, `SUSPENDED`, `ACTIVATION_EXPIRATION_DATE`, `ACTIVATION_NOTIFICATION_DATE`, `ROAMING_PARTNER_REMOVAL_NOTIF`, `VALIDITY_START_DATE`, `CONSUMPTION_ORDER`, `CONSUMPTION_NOTIF`, `ALLOWANCE_VOLUME`, `ALLOWANCE_TERM`, `ALLOWANCE_TERM_UNIT`, `NETWORK_GROUP_ID`, `UUID`, `CLIENT_TYPE`, `NETWORK_CHANGE_ACTIVATION_ENABLED`, `CAP_ID`, `ENTERPRISE_PURCHASE_ID`, `DATA_CAP_TYPE`, `DEVICE_PLAN`)  SELECT @user_id,@data_id , 13, @next_renewal, 0, 0, NULL, 7, NULL, NULL, NULL, NULL, NULL, 1, NULL, NULL, now(), now(), NULL, 0, NULL, NULL, 0, NULL, 0, 0, 0, 0,  now(), NULL, 0,  now(), 0, 0, NULL, NULL, NULL, NULL, @unique_uuid, clientTypeValue, 0, tsim_card_pool_data_cap.id, NULL, NULL, 1 FROM tdevice_plan inner JOIN tsim_card_pool_data_cap ON tsim_card_pool_data_cap.SIM_CARD_POOL_ID=sms_id where planId=devicePlanId;
            ...
            if @plan_type ='PoolPlan' then
				INSERT INTO `tuser_purchase` (`USER_ID`, `TYPE_ENTRY_ID`, `USER_PURCHASE_TYPE`, `EXPIRATION_DATE`, `RECURRING`, `PAYMENT_RETRY_COUNT`, `PAYMENT_REFUSE_DATE`, `TYPE`, `VOLUME`, `DATA_BUCKET_ID`, `PURCHASE_BUNDLE_ID`, `PURCHASE_BUNDLE_TYPE`, `MASTER_BUNDLE_ID`, `ACTIVE`, `TRANSACTION_ID`, `ENTERPRISE_ID`, `CREATED`, `ACTIVATION_DATE`, `PRIMARY_USER_PURCHASE_ID`, `RECURRING_COUNTER`, `SELECTED_NETWORK_GROUP_ID`, `THROTTLING_DATA_BUCKET_ID`, `RESET_VALIDITY_TERM_ENABLED`, `RESET_DATE`, `CAPPED`, `EXPIRED_NOTIF`, `ACTIVATION_NOTIF`, `SUSPENDED`, `ACTIVATION_EXPIRATION_DATE`, `ACTIVATION_NOTIFICATION_DATE`, `ROAMING_PARTNER_REMOVAL_NOTIF`, `VALIDITY_START_DATE`, `CONSUMPTION_ORDER`, `CONSUMPTION_NOTIF`, `ALLOWANCE_VOLUME`, `ALLOWANCE_TERM`, `ALLOWANCE_TERM_UNIT`, `NETWORK_GROUP_ID`, `UUID`, `CLIENT_TYPE`, `NETWORK_CHANGE_ACTIVATION_ENABLED`, `CAP_ID`, `ENTERPRISE_PURCHASE_ID`, `DATA_CAP_TYPE`, `DEVICE_PLAN`)  SELECT @user_id,tdevice_plan.data_id , 13, @next_renewal, 0, 0, NULL, 5, NULL, NULL, NULL, NULL, NULL, 1, NULL, NULL,  now(),  now(), NULL, 0, NULL, NULL, 0, NULL, 0, 0, 0, 0,  now(), NULL, 0,  now(), 0, 0, NULL, NULL, NULL, NULL, @unique_uuid, clientTypeValue, 0, tsim_card_pool_data_cap.id, NULL, NULL, 1 FROM tdevice_plan inner JOIN tsim_card_pool_data_cap ON tsim_card_pool_data_cap.SIM_CARD_POOL_ID=data_id where planId=devicePlanId;
				update tsim_card_pool_data_cap  set AVL_VOLUME=AVL_VOLUME+@volume, CAP=CAP+@volume ,volume=volume+@volume ,EXPIRATION_DATE=@next_renewal  where SIM_CARD_POOL_ID = @data_id ;
          end if;
...
-- User purchase end
    ...
    update migration_asset asset set asset.status=b'1' where asset.imsi=imsi;
    
    IF LOWER(userStatus) ='suspended' THEN
		update tuser_sim_card set SIM=6 where user_id=@user_id;
    END IF; 
	END IF;
	COMMIT;
    SET successCount = 1;
