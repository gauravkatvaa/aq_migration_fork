DROP procedure IF EXISTS `AP_DISCOUNT_MIGRATION_PROCEDURE`;
DELIMITER //
CREATE PROCEDURE AP_DISCOUNT_MIGRATION_PROCEDURE(IN p_accountId int,IN p_invoiceDate date,IN p_renewalDate date,
...
BEGIN
    ...
    IF (_status_code>0) THEN
        BEGIN
            ...
            --    select concat('check status1',_tariffId,_discountType,_type,_discountGlcodeId,_price, _vasStartDate, _vasEndDate);
                IF (done = 1) THEN
                    SET p_status='SUCCESS';
                    --  select 'Finished';
                        leave cursorloop;
                END IF;             
-- select concat('check status2',_tariffId,_discountType,_type,_discountGlcodeId,_price, _vasStartDate, _vasEndDate);
