...
logging.basicConfig(filename='sim_alias.log', level=logging.INFO,
...
def add_reason(row):
    ...
    :param row: field of the csv files
    :return: reason and append new column reason in the csv files for the application
             errors and the validation errors
    """
    reasons = []
    ...
    if not pd.notna(row['alias']):
        reasons.append('alias is missing')
    if reasons:
        return ', '.join(reasons)
    else:
        return 'Ok'
...
for csv_file in csv_files:
    ...
    for index, row in combined_df.iterrows():
        ...
        device_id = str(row["alias"])

        if pd.notna(device_id):
            if (imsi, msisdn) not in existing_pairs:
                unmatched_rows.append(row)
            else:
                updates.append({"imsi": imsi, "msisdn": msisdn, "device_id": device_id})
