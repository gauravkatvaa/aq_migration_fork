...
logs = {
    "migration_info_logger":f"stc_ingestion_billing_bss_info_{current_time_stamp}.log",
    "migration_error_logger":f"stc_ingestion_billing_bss_error_{current_time_stamp}.log"
}
...
def billing_mapping_prerequisite():
    # file_path = os.path.join(dir_path, "Billing_BSS_common_csv_file", "data_df_assets.csv")
    file_path = billing_file_path['bss_common_csv']

    if True or not os.path.exists(file_path):

        data_df_assets = pd.read_csv(billing_file_path['file_migration_assets'], skipinitialspace=True, keep_default_na=False, dtype=str)
        # print("data_df_assets",data_df_assets)
        ...
        print("data_df_assets2", data_df_assets)
        select_id_dvc_svc_query = f"SELECT ID,IMSI, ENT_ACCOUNTID, DEVICE_PLAN_ID, SERVICE_PLAN_ID FROM assets"
        ...
        data_df_assets.to_csv(file_path,  index=False)
...
def billing_mapping():
    billing_mapping_prerequisite()
    data_df_assets=pd.read_csv(billing_file_path['bss_common_csv'], skipinitialspace=True, keep_default_na=False, dtype=str)
    ...
    ####this functionneed to uncomment when you will the changes the device plan and device plan name
    # data_df_device_plan=update_ent_account_and_device_plan(data_df_device_plan, update_dict)
    print("lenof_assest_before checking",len(data_df_assets))
    data_df_assets.to_csv("assest_with_billing.csv")
    ...
    # data_df_assets = data_df_assets[
    # (~data_df_assets["device_id"].isin([""]) & ~data_df_assets["service_plan_id"].isin([""])) | 
    # (data_df_assets["state"] == "Terminate")]

    print("lenof_assest_aftere checking",len(data_df_assets))

    # Convert column names to lowercase
    data_df_device_plan.columns = data_df_device_plan.columns.str.lower()

    # Apply the split operation on the 'name' column and take the part before the underscore
    # data_df_device_plan['name'] = data_df_device_plan['name'].str.split('_').str[0]
    ...
    for index, row in  data_df_device_plan.iterrows():
        try:
            ...
            # and account_id in (SELECT ID from accounts WHERE NOTIFICATION_UUID='{row['billing_account_id']}') and device_rate_plan.DELETED=0;"""

            # Extract the part of plan_name before the underscore
            
            select_dvc_tmp_ID_query = f"""SELECT id AS device_plan_id, REPLACE(JSON_EXTRACT(extra_metadata, '$.templateId'), '"', '') AS templateId,
            PLAN_TYPE AS type FROM device_rate_plan WHERE plan_name = '{row['name']}' AND account_id IN (SELECT ID FROM accounts WHERE NOTIFICATION_UUID = '{row['billing_account_id']}') AND device_rate_plan.DELETED = 0;"""

            print("query with data ",select_dvc_tmp_ID_query)

            logger_info.info(f"query with data {select_dvc_tmp_ID_query}")
    ...
    # tarrifs_ids_querry=f"""select a.PLAN_ID, a.CHARGE_SPEC_ID, a.CHARGE_CATEGORY, a.gl_code_id, a.action_type,     a.is_proration,a.PARENT_CHARGESPEC_ID , b.disc_id, b.disc_gl_code_id, p.apn_type,a.SERVICE_CATEGORY from 
    # (select pc .PLAN_ID as PLAN_ID , pc.CHARGE_SPEC_ID as CHARGE_SPEC_ID ,cs.CHARGE_CATEGORY as 
    # CHARGE_CATEGORY,cs.GL_CODE_ID as gl_code_id ,cs.ACTION_TYPE as action_type ,cs.IS_PRORATION as is_proration,cs.SERVICE_CATEGORY as SERVICE_CATEGORY, cs.PARENT_CHARGESPEC_ID
    # from plan_chargespecs pc inner join charging_spec cs on pc .CHARGE_SPEC_ID = cs .CHARGE_SPEC_ID  where cs .LIFE_CYCLE_STATUS = 'LIVE' and pc.plan_id in 
    # ( select plan_id from plan where LIFE_CYCLE_STATUS = 'LIVE') and is_enabled is true  and pc.LIFE_CYCLE_STATUS='LIVE') a left join 
    ...
    # dc.LIFE_CYCLE_STATUS = 'LIVE' and d.LIFE_CYCLE_STATUS = 'LIVE' and d.GL_CODE_ID is not null) 
    # b  on a.charge_spec_id =b.charge_spec_id left join plan p on p.plan_id=a.plan_id and p.LIFE_CYCLE_STATUS='LIVE';"""
    # id_dvc_tmp = sql_query_fetch(tarrifs_ids_querry, product_catalogue, logger_error)

    # data_df_tarrifs_ids = pd.DataFrame(id_dvc_tmp, columns=["plan_id","charge_spec_id","charge_category","gl_code_id","action_type","is_proration", "parent_csid","discount_id", "discount_glcode_id", "APN_TYPE","service_category"])
    # #plan_id

    # data_df_tarrifs_ids['is_proration'].fillna(0, inplace=True)
    # data_df_tarrifs_ids['parent_csid']= data_df_tarrifs_ids["parent_csid"].apply(lambda x: int(x) if x != '' else x)

    # data_df_tarrifs_ids["is_proration"] = data_df_tarrifs_ids["is_proration"].apply(lambda x: int(x) if x != '' else x)
        # Execute the SQL query
    tarrifs_ids_querry = """ 
    SELECT a.PLAN_ID, a.CHARGE_SPEC_ID, a.CHARGE_CATEGORY, a.gl_code_id, a.action_type, 
        a.is_proration, a.PARENT_CHARGESPEC_ID, b.disc_id, b.disc_gl_code_id, 
        p.apn_type, a.SERVICE_CATEGORY 
    FROM (
        SELECT pc.PLAN_ID, pc.CHARGE_SPEC_ID, cs.CHARGE_CATEGORY, cs.GL_CODE_ID AS gl_code_id, 
            cs.ACTION_TYPE AS action_type, cs.IS_PRORATION AS is_proration, 
            cs.SERVICE_CATEGORY, cs.PARENT_CHARGESPEC_ID 
        FROM plan_chargespecs pc 
        INNER JOIN charging_spec cs ON pc.CHARGE_SPEC_ID = cs.CHARGE_SPEC_ID  
        WHERE cs.LIFE_CYCLE_STATUS = 'LIVE' 
        AND pc.plan_id IN (SELECT plan_id FROM plan WHERE LIFE_CYCLE_STATUS = 'LIVE') 
        AND pc.is_enabled = TRUE 
        AND pc.LIFE_CYCLE_STATUS = 'LIVE'
    ) a 
    LEFT JOIN (
        SELECT dc.CHARGING_SPEC_ID AS CHARGE_SPEC_ID, dc.discount_id AS disc_id, 
            d.gl_code_id AS disc_gl_code_id 
        FROM discount_chargespec dc 
        LEFT JOIN discount d ON dc.discount_id = d.discount_id 
        WHERE dc.is_enabled = TRUE 
        AND dc.LIFE_CYCLE_STATUS = 'LIVE' 
        AND d.LIFE_CYCLE_STATUS = 'LIVE' 
        AND d.GL_CODE_ID IS NOT NULL
    ) b ON a.charge_spec_id = b.charge_spec_id 
    LEFT JOIN plan p ON p.plan_id = a.plan_id AND p.LIFE_CYCLE_STATUS = 'LIVE';
    """
    ...
    columns = [
        "plan_id", "charge_spec_id", "charge_category", "gl_code_id", "action_type", 
        "is_proration", "parent_csid", "discount_id", "discount_glcode_id", "APN_TYPE", "service_category"
    ]
    data_df_tarrifs_ids = pd.DataFrame(id_dvc_tmp, columns=columns)

    # Data Cleaning and Type Conversion
    data_df_tarrifs_ids["is_proration"].fillna(0, inplace=True)

    # Convert 'parent_csid' and 'is_proration' to integers, handling NaNs correctly
    data_df_tarrifs_ids["parent_csid"] = pd.to_numeric(data_df_tarrifs_ids["parent_csid"], errors='coerce').fillna(0).astype(int)
    data_df_tarrifs_ids["is_proration"] = pd.to_numeric(data_df_tarrifs_ids["is_proration"], errors='coerce').astype(int)
    ...
    truncate_mysql_table(billing_db_configs, "migration_service_apn_details", logger_error)

    print("test", len(data_df_assets))
    # Generate a string of unique account IDs for the SQL query
    unique_account_ids = '("' + '","'.join(data_df_assets['device_id'].astype(str).unique()) + '")'

    # Construct the SQL query using the unique account IDs
    # query_service_apn_details = f"""SELECT APN_NAME, ACCOUNT_ID, APN_ID,APN_TYPE FROM service_apn_details WHERE ACCOUNT_ID IN {unique_account_ids};"""
    query_service_apn_details = f"""SELECT distinct( dpsam.DEVICE_PLAN_ID_ID),sad.APN_NAME,sad.ID AS APN_ID,sad.APN_TYPE
    FROM service_apn_details sad JOIN device_plan_to_service_apn_mapping dpsam on sad.ID = dpsam.APN_ID WHERE dpsam.DEVICE_PLAN_ID_ID IN {unique_account_ids};"""
    service_apn_details_df=fetch_param_as_df(query_service_apn_details,aircontrol_db_configs,logger_info,logger_error)
    
    service_apn_details_df.columns =['DEVICE_ID','APN_NAME', 'APN_ID','APN_TYPE']
    print(query_service_apn_details,service_apn_details_df)
    data_df_service_apn_details_df, data_service_apn_details_df = insert_batches_df_into_mysql(service_apn_details_df, billing_db_configs, "migration_service_apn_details",   logger_info, logger_error)
...
def run_bulk_registration(jar_path):
    try:
        log_file = os.path.join(jar_path, "bulk_registration.log")

        # Define the environment variable export command
        export_command = f"export SPRING_CONFIG_LOCATION={jar_path}application.properties"

        # JAR execution command
        jar_command = (
            f"nohup java -jar {jar_path}BulkRegistrationBatchProcessor-0.0.1-SNAPSHOT.jar "
            f"--spring.config.location=file:{jar_path}application.properties "
            f"> {log_file} 2>&1 &"
        )

        # Combine both commands
        full_command = f"{export_command} && {jar_command}"

        print("Command >>>", full_command)  # Debugging
        
        process = subprocess.Popen(full_command, shell=True, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)

        logger_info.info(f"JAR process started with PID: {process.pid}")
        return process

    except Exception as e:
        logger_error.error(f"Failed to start JAR process: {str(e)}")
        return None
...
#         print("Command >>>", command)  # Print for debugging
...
def monitor_logs(process):
    """Continuously monitors the log output from the JAR process."""
    print("Monitoring logs...")
    
    if not process:
        logger_error.error("Process is None. Exiting log monitoring.")
        return

    while process.poll() is None: 
        output = process.stdout.readline()
        if output:
            logger_info.info(output.strip())
        
        error = process.stderr.readline()
        if error:
            logger_error.error(error.strip())
        
        time.sleep(2)
    
    logger_info.info("JAR process has completed.")


def bss_mapping():
    billing_mapping_prerequisite()
    data_df_assets = pd.read_csv(billing_file_path['bss_common_csv'],
    ...
    # data_df_assets = data_df_assets[~data_df_assets["device_id"].isin([""])]
    # data_df_assets = data_df_assets[~data_df_assets["service_plan_id"].isin([""])]
    # data_df_assets = data_df_assets[~(data_df_assets["state"] == "Terminate")]
    data_df_assets = data_df_assets[~data_df_assets["state"].isin(["Terminate", "Retired"])]
    ...
    print(f"<<< bss_jar_path >>>> {bss_jar_path}")
    ...
    run_bulk_registration(bss_jar_path)
