...
def fetch_account_id_cu(account_id):
    querry = f"""select id from accounts where LEGACY_BAN = '{account_id}' and TYPE = 4 and LOCKED != 1 and DELETED != 1;"""
...
def fetch_account_id_bu(account_id):
    querry = f"""select id from accounts where LEGACY_BAN = '{account_id}' and TYPE = 5 and LOCKED != 1 and DELETED != 1;"""
    data = sql_query_fetch(querry, aircontrol_db_configs, logger_error)
    if data:
        data = data[0][0]
        return data
...
def api_validate_user_csr(username, password):
    payload = f"username={username}&password={password}&uuid=2c4ec689-389a-436c-ba12-f5905b881edf"
    headers = {
    'Accept': 'application/json, text/plain, */*',
    'Accept-Language': 'en-GB,en-US;q=0.9,en;q=0.8',
    'Cookie': 'SameSite=none; SameSite=none; SameSite=none; JSESSIONID=90BB837FA952137A3BE1E93164DF776B',
    'Content-Type': 'application/x-www-form-urlencoded'
    }

    url = api_validate_url
    print("api validate url",url)
    token = None
    ...
    try:
        ...
        if "token" in response_data:
            ...
            logger_info.info("Token retrieved successfully: {}".format(token))
        else:
            logger_error.error("Token not found in response.")
...
def tarrif_plan(token, pc_tariff_plan_name):
    payload = {}
    # account_id = 2
    token = f'Bearer {token}'
    headers = {
    ...
    # print(data)
    for lst in data:
        if lst["name"] == pc_tariff_plan_name:
            return lst
...
def csr_get_publish_zone_group(token,publish_zone_group_url,zone_group_id):
    csr_publishzone_group_url = publish_zone_group_url + zone_group_id
    # csrAddOnPlanAccountPlanUrl = quote(csrAddOnPlanAccountPlanUrl, safe=':/?&=')
    payload = {}
    token = f'Bearer {token}'
    headers = {
    ...
    response = requests.request("GET", csr_publishzone_group_url, headers=headers, data=payload, verify=False)
    ...
    logger_info.info("Publish Zone group Url \n {} ".format(csr_publishzone_group_url))
    # logger_info.info("payload \n {} ".format(payload))
    logger_info.info("Publish zone group status {} content {}".format(response.status_code, content))
